def it_is_valid(cont_passwords, given_cont, given_pass):
    for key, value in cont_passwords.items():
        if key == given_cont and value == given_pass:
            return True
    return False
...
def student_and_highest_score(cont_students):
    best_student = ""
    best_score = 0
    for student, scr in cont_students.items():
        total_score = 0
